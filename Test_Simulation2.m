%% Test Simulation2
% Time-specific information storage with innovation variance which varyies as a
% periodic square waveform (scenario 2 - Fig.1b main document)
close all;clear all;clc
%% generation of time variant innovation variance
rng(1,'twister')
...
DC=50;                              % duty-cycle
c = -cmax*square(2*pi*f_osc*t,DC);  % innovation variance varying as square periodic waveform
A = 0.8;                          % value of AR parameter
Su_T(1,1,:)=c+cmax*2;               % theoretical vector of innovation variance 
A_T(1,1,:) = repmat(A,1,1,length(c)); % theoretical AR coefficient set to 0.8               
% Data generation
